...
void ArrayList<e>::removeAt(int i) {
    if(i < 0 || i > numOfElements) {
        ...
        for(int j=i; j<numOfElements-i-1; j++) {
            array[j] = array[j+1];
        }
        numOfElements--;
    } 
...
template<typename e>
e ArrayList<e>::findMinRecursive(ArrayList<e> arr, e min) {
    ...
    if(arr.retrieveAt(0) < min) {
        min = arr.retrieveAt(0);
    }

    arr.removeAt(0);
    return findMinRecursive(arr, min);

}

template <typename e>
unsigned int  ArrayList<e>::seqSearchIter(e element) {
...
template <typename e>
unsigned int  ArrayList<e>::seqSearchRec(e element, int i) {
