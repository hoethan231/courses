...
template <typename T>
T LinkedList<T>::retrieveAt(int i) {
...
template<typename T>
T LinkedList<T>::findMinRecursive(LinkedList<T> list, T min) {

    if(list.isEmpty()) {
        return min;
    }

    if(list.retrieveAt(0) < min) {
        min = list.retrieveAt(0);
    }

    list.removeAt(0);

    return findMinRecursive(list, min);

}

template <typename T>
bool LinkedList<T>::seqSearchIter(T val) {
    Node<T>* current = head;
...
template <typename T>
bool LinkedList<T>::seqSearchRec(T val, Node<T>* current) {
    if (current == nullptr) {
        current = head;
    }
    if (current == nullptr) {
    ...
    return seqSearchRec(val, current->next);
}